...
class NqDataLoader:
    ...
    def sanityCheck(self):
        ...
        nonConsTmpLbl = np.zeros(len(self.dataTimeStart)) == 0
        # start with all False labels
        nonConsLbl = np.zeros(len(self.dataTimeStart)) > 0
...
#            if data.dtype == np.int64: # Sleep inertia format
            if impType == 'si':
                data = np.genfromtxt(fileIn, dtype=int,
                ...
                # TO REMOVE
                # Just to make sanity work
                self.dataKeys = np.zeros(len(self.dataHT))
                remNum = self.sanityCheck()
                # print remNum
            else:  # PD format
                data = np.genfromtxt(fileIn,dtype=None,encoding = 'ASCII',
                                     delimiter=',', skip_header=0)
                # load
            ...
            # load flight time
            self.dataFT = np.array([self.dataTimeStart[i]-self.dataTimeStart[i-1]
    ...
    def getStdVariablesFilt(self,fileIn, impType=None):
        ...
        nqObj.filtData(nqObj.FLT_NO_MOUSE |
                       nqObj.FLT_NO_LONG_META | nqObj.FLT_NO_BACK)
...
def genFileStruct(dataDir, maxRepNum=4):
    ...
    for f in files:
        ...
        if(m):  # file found
            ...
            dateMap[pID][repID][expID] = datetime.datetime.fromtimestamp(
                int(timeStamp))
        else:
            print (f , ' no')
